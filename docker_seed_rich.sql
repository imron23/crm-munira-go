...
UPDATE leads SET lost_reason = (ARRAY['Kendala Budget / Kemahalan','Pilih Travel Lain','Ghosting / Tidak Respon'])[floor(random()*3)+1]
WHERE status_followup = 'Lost';
...
UPDATE leads SET program_id = (SELECT id FROM programs ORDER BY random() LIMIT 1),
                 assigned_to = (SELECT id::text FROM admin_users ORDER BY random() LIMIT 1);
